...
const AdminLogin = () => {
  ...
  const { setSession, isAuthenticated, isAdmin } = useSessionContext();
  ...
  const onSubmit = async (data) => {
    // Rate limiting check
    const userKey = `admin_login_${data.username}`;
    ...
    try {
      const response = await secureApi.post("/api/admin/admin", sanitizedData);
  ...
  const handleMFASubmit = async () => {
    ...
    try {
      const response = await secureApi.post("/api/admin/admin", {
  ...
  if (requiresMFA) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-100 to-white flex items-center justify-center px-4">
        ...
        >
          <h2 className="text-2xl font-bold text-center mb-6 text-gray-800">Admin Two-Factor Authentication</h2>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-white flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="mt-4 text-center text-xs text-gray-500">
          <p>🔒 Admin access protected with:</p>
